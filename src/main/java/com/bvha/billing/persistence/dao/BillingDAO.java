...
public class BillingDAO {
	...
	public Map getBillingByStatus(String status) throws Exception {
		...
		try (SqlSession session = SQLConnectionFactory.getSession().openSession()) {
			Map period = (Map) session.selectOne("com.bvha.billing.persistence.mapper.billing.CheckForExisting",
					status);
			if (period == null) {
				return null;
			}
			bill.put("period", period);
			bill.put("list", session.selectList("com.bvha.billing.persistence.mapper.billing.getDraftBillingDetails",
					period.get("id")));
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
